...
export default function GroupsListPage() {
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setJoinError(err?.response?.data?.detail?.message || err?.message || "Failed to join");
    } finally {
      setJoining(false);
    }
  };
        ...
        <CardContent>
          <form onSubmit={handleJoin} className="flex flex-col gap-2 sm:flex-row sm:items-end">
            <div className="flex-1 flex flex-col gap-1.5">
              ...
              <Input id="invite" required value={inviteCode} onChange={(e) => setInviteCode(e.target.value)} placeholder="8-character code" />
            </div>
            <Button type="submit" disabled={joining || !inviteCode.trim()}>{joining ? "Joining..." : "Join"}</Button>
          </form>

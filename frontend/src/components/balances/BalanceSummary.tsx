            ...
            <CardContent className="flex items-center justify-between py-4">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  paid {formatMoney(b.total_paid, currency)} · owes{" "}
                  {formatMoney(b.total_owed, currency)}
                </p>
              </div>
              <span
                className={`text-lg font-semibold ${positive ? "text-emerald-600" : negative ? "text-red-600" : "text-slate-500"}`}
              >
                {positive ? "+" : ""}
